...
*/
  ...
  // Enqueues the elements from the subtree rooted at node into q in postOrder.
  {
	...
	//This method display elements in breadth-first order
   ...
   private void LevelOrder(BSTNode<T> node)
    {
	LinkedQueue<BSTNode> q = new LinkedQueue<BSTNode>();
      if (node != null)
      {
        q.enqueue(node);
...
//This method count of all the values less than or equal to an argument value
  public int countLess(T maxValue)
  {
	  return recCountLess(maxValue,root);
  }

  private int recCountLess(T maxValue,BSTNode<T> node)
  	{
		if(node==null)
		return 0;
		else if (comp.compare(maxValue,node.getInfo()) >= 0)
		return 1 + recCountLess(maxValue,node.getLeft()) + recCountLess(maxValue,node.getRight());
		else
		return 0 + recCountLess(maxValue,node.getLeft());
  	}

	//This method count of all the values greater than or equal to an argument value
  	 public int countGreater(T minValue)
	  {
		  return recCountGreater(minValue,root);
	  }

	  private int recCountGreater(T minValue,BSTNode<T> node)
	  	{
			if(node==null)
			return 0;
			else if (comp.compare(minValue,node.getInfo()) <= 0)
			return 1 + recCountLess(minValue,node.getLeft()) + recCountGreater(minValue,node.getRight());
			else
			return 0 + recCountGreater(minValue,node.getRight());
  	}
	...
	//This method determines the height of the tree
	 ...
	 private int recHeight(BSTNode<T> node)
	 {
		...
		{
			int countLeft = 1 + recHeight(node.getLeft());
			int countRight = 1+ recHeight(node.getRight());
 ...
 //This method provides the ration of minimum and actual tree height
	public double fRatio()
	{
		...
		{
		double minHeight =((int)( Math.log(size())/Math.log(2)));
		int actualHeight = height();

		double ratio = minHeight / actualHeight;
		return ratio;
		}
	}
